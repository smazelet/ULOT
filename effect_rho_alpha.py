...
def plot_3d_graph_pair(data: Data, plan, node_size=100, edge_color='gray', title=None, ax=None, min=None, max=None, xlim=None, ylim=None,zlim=None):
    """
    ...
    for i, j in edge_index_s.T:
        ax.plot(
            ...
            [pos_s[i, 1], pos_s[j, 1]], 
            [pos_s[i, 2], pos_s[j, 2]], 
            color=edge_color, alpha=0.5, linewidth=0.7
        )
    ...
    ax.scatter(pos_s[:, 0], pos_s[:, 1], pos_s[:, 2], 
               s=node_size, c=node_colors_s, depthshade=True, alpha=0.4,linewidths=1,edgecolors='black')
    
    # Draw nodes with RGB colors
    ax.scatter(pos_t_[:, 0], pos_t_[:, 1], pos_t_[:, 2], 
               s=node_size, c=node_colors_t, depthshade=True, alpha=0.4,linewidths=1,edgecolors='black')
    ...
    for i in range(plan.shape[0]):
        for j in range(plan.shape[1]):
            ax.plot(
                ...
                color=edge_color, alpha=0.5, linewidth=plan[i,j]
            )
...
dataset_2, pos_g1, pos_g2, G1_nx, G2_nx, D1, D2, M, rgb1, rgb2=create_sbm_2_clusters(n_graphs=1,nc1_i=2,nc2_i=2,clusters1=[1,2],clusters2=[2,3], random_graph=False, noise=0.3, p_intra=0.5, p_inter=0.05,n_nodes1=20,n_nodes2=20)
...
for i, rho in enumerate(rhos[::-1]):
    # if i==0 or i==len(rhos)//2 or i==len(rhos)-1:
        ...
        batched_alphas_s = alpha[batch_indices_s].reshape(
            len(alpha[batch_indices_s]), 1
        )
        batched_alphas_t = alpha[batch_indices_t].reshape(
            len(alpha[batch_indices_t]), 1
        )
...
dataset_3, pos_g1, pos_g2, G1_nx, G2_nx, D1, D2, M, rgb1, rgb2=create_sbm_3_clusters(n_graphs=10,nc1_i=3,nc2_i=3,clusters1=[1,2,3],clusters2=[1,2,3], random_graph=False, noise=0.4, p_intra=0.5, p_inter=0.05,n_nodes1=30,n_nodes2=30)

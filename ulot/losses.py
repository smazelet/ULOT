...
def w_loss(P, x1, x2, batch1=None, batch2=None, mask1=None, mask2=None, normalize_cost=True):
    """
...
def FUGW(P, x1, x2,alphas,rhos,batch1=None, batch2=None, mask1=None, mask2=None, geodesic=False, connectivity_s=None, connectivity_t=None):
    """
    ...
    """
    wasserstein_loss = w_loss(P, x1, x2,batch1=batch1, batch2=batch2, mask1=mask1, mask2=mask2)
    gromov_loss = gw_loss(P,batch1, batch2, mask1, mask2, connectivity_s, connectivity_t)
    ...
    marginals= 2*sum_P*kl_div(P_marginal_1.squeeze()*mask1, p, mass=False, batch=True) + 2*sum_P*kl_div(P_marginal_2.squeeze()*mask2, q, mass=False, batch=True)+(1-sum_P**2)+(1-sum_P**2)

    return {"total": (1-alphas.squeeze())  * wasserstein_loss + alphas.squeeze()*gromov_loss + rhos.squeeze() * marginals, "wasserstein": wasserstein_loss, "gromov": gromov_loss, "marginals": marginals}

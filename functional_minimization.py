...
dataset=SBM(n_graphs=1,nc1_i=3,nc2_i=2,clusters1=[1,2,3],clusters2=[1,2], random_graph=False, noise=0.3, p_intra=0.6, p_inter=0.05) 
...
G_along_step_01=np.zeros((n_steps,graph_pair_batch.connectivity_s.shape[0],graph_pair_batch.connectivity_s.shape[1]))
...
G_along_step_05=np.zeros((n_steps,graph_pair_batch.connectivity_s.shape[0],graph_pair_batch.connectivity_s.shape[1]))
edge_index_flow=G1.edge_index
...
for step in steps_01:
    ...
    max=np.max([torch.max(graph_pair.x_s),torch.max(graph_pair.x_t),torch.max(torch.tensor(F))])
    min=np.min([torch.min(graph_pair.x_s),torch.min(graph_pair.x_t),torch.min(torch.tensor(F))])
...
torch.manual_seed(40)
np.random.seed(6)
steps_01=[0,400,800,1200,1600]
steps_05=[0,400,800,1200,1600]

G_0=G_flows_01[0]
graph=Data(x=G_0.x, edge_index=G_0.edge_index)
G_0_nx= to_networkx(G_0, to_undirected=True)
min_rgb=np.min([torch.min(G_0.x),np.min([torch.min(G_flows_01[i].x) for i in range(len(steps_01))]),np.min([torch.min(G_flows_05[i].x) for i in range(len(steps_05))])])

max_rgb=np.max([torch.max(G_0.x),np.max([torch.max(G_flows_01[i].x) for i in range(len(steps_01))]),np.max([torch.max(G_flows_05[i].x) for i in range(len(steps_05))])])

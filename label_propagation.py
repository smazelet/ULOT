...
for k in tqdm(range(n_graphs)):
    ...
    for n in range(n_steps):
        ...
        if n>1 and torch.abs(rhos_2[k,n]-rhos_2[k,n-1])<eps and torch.abs(alphas_2[k,n]-alphas_2[k,n-1])<eps:
            break
        ...
        loss = F.kl_div(log_probs[validation_nodes], one_hot_matrix_t[validation_nodes], reduction='batchmean')
...
for k in tqdm(range(n_graphs)):
    ...
    for n in range(n_steps):
        if n>1 and torch.abs(rhos_3[k,n]-rhos_3[k,n-1])<eps and torch.abs(alphas_3[k,n]-alphas_3[k,n-1])<eps:
            break
...
for i in range(n_graphs):
    ...
    ax.autoscale()
    if i==0:
       ax.plot(x[0], y[0], marker='o', markersize=6, color='green', label='Initialization',linestyle='None', zorder=10)
       ax.plot(x[-1], y[-1], marker='o', markersize=2, color='black', label='Convergence',linestyle='None',zorder=10)
    else:
       ax.plot(x[-1], y[-1], marker='o', markersize=2, color='black',zorder=10)

ax.set_title("Optim. $(\\alpha,\\rho)$ with respect \n to KL for $(1,2,3) \\rightarrow (1,2)$ ")
ax.set_ylim(0,1)
ax.set_xlim(0.003,1)
ax.set_xscale('log')
ax.set_xlabel(r"$\rho$")

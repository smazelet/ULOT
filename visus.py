...
for i, alpha in enumerate(alphas):
    ...
    ax.set_xlim(-1.5, 5)
    ax.text(
        0.5, 0, r"$\alpha$" + f"={alpha}", transform=ax.transAxes, ha="center", va="top"
    )
...
for i, rho in enumerate(rhos[::-1]):
    ...
    ax.set_xlim(-1.5, 5)
    ax.text(
        0.5, 0, r"$\rho$" + f"={rho:.3f}", transform=ax.transAxes, ha="center", va="top"
    )
...
with warnings.catch_warnings():
    ...
    solver_losses_all_sbm = np.load(path_solver_losses_sbm, allow_pickle=True).item()
...
solver_losses = solver_losses_all_sbm["losses_test_solver"]
color = [
...
mask = ~np.isnan(solver_losses)
solver_losses = np.array(solver_losses)
...
pl.scatter(
    ...
    alpha=0.5,
    c=color,
    label=f"pearson correlation={pearsonr(solver_losses[mask], losses[mask]).statistic:.2f}",
    vmin=0,
    vmax=1,
)
...
min_rgb = np.min(
    [
        torch.min(G_0.x),
        np.min([torch.min(G_flows_01[i].x) for i in range(len(steps_01))]),
        np.min([torch.min(G_flows_05[i].x) for i in range(len(steps_01))]),
        torch.min(G_2.x),
    ]
)
...
max_rgb = np.max(
    [
        torch.max(G_0.x),
        np.max([torch.max(G_flows_01[i].x) for i in range(len(steps_01))]),
        np.max([torch.max(G_flows_05[i].x) for i in range(len(steps_05))]),
        torch.max(G_2.x),
    ]
)
